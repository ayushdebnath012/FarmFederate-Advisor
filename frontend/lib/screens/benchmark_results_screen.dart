...
class BenchmarkResultsScreen extends StatelessWidget {
    ...
    return Container(
      ...
      child: const Column(
        children: [
          ...
          SizedBox(height: 4),
          Text(
            'Federated LLM | Federated ViT | Federated VLM | Federated RAG\nK=3 clients | T=8 rounds | E=3 local epochs | FedAvg | avg 102.1% retention',
  ...
  Widget _buildParadigmTable() {
    return _buildTableCard(
      ...
      rows: const [
        ['Fed. LLM', 'BERT-tiny', '0.460', '107.7%'],
        ['Fed. ViT', 'EfficientNet', '0.886', '100.0%'],
        ['Fed. VLM', 'CLIP', '0.861', '98.6%'],
      ],
      bestRow: 2,
    );
  }
  ...
  Widget _buildLLMTable() {
    return _buildTableCard(
      title: 'LLM Encoders - Centralized Benchmark',
      subtitle: 'Text paradigm (5 encoders, centralized training)',
      color: const Color(0xFF4169E1),
      headers: const ['Model', 'Cent. F1', 'Diversity'],
      rows: const [
        ['BERT-tiny', '0.487', '100%'],
        ['RoBERTa-tiny', '0.463', '100%'],
        ['ALBERT-tiny', '0.450', '100%'],
        ['DistilBERT', '0.433', '100%'],
        ['MobileBERT', '0.417', '100%'],
      ],
      bestRow: 0,
    );
  }
  ...
  Widget _buildViTTable() {
    return _buildTableCard(
      title: 'ViT Encoders - Centralized Benchmark',
      subtitle: 'Image paradigm (5 encoders, centralized training)',
      color: const Color(0xFF228B22),
      headers: const ['Model', 'Cent. F1', 'Diversity'],
      rows: const [
        ['EfficientNet', '0.911', '100%'],
        ['ConvNeXT-tiny', '0.899', '100%'],
        ['DeiT-tiny', '0.873', '100%'],
        ['Swin-tiny', '0.873', '100%'],
        ['ViT-Base', '0.861', '100%'],
      ],
  ...
  Widget _buildVLMTable() {
    return _buildTableCard(
      title: 'VLM Fusion Strategies - Centralized Benchmark',
      subtitle:
          'Multimodal paradigm (8 fusion strategies, centralized training)',
      color: const Color(0xFFB22222),
      headers: const ['Fusion', 'Cent. F1', 'Params (M)'],
      rows: const [
        ['CLIP', '0.949', '15.7M'],
        ['BLIP-2', '0.937', '15.9M'],
        ['Cross-Attention', '0.924', '15.9M'],
        ['Flamingo', '0.911', '16.1M'],
        ['Unified-IO', '0.911', '17.2M'],
        ['Gated', '0.899', '15.8M'],
        ['CoCa', '0.899', '16.2M'],
        ['Concat', '0.886', '15.6M'],
      ],
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              ...
              const Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Federated RAG Advisory',
                        style: TextStyle(
                            color: Colors.white,
                            fontWeight: FontWeight.bold,
                            fontSize: 15)),
                    Text(
                        'Privacy-preserving retrieval + treatment generation (10 rounds)',
          ...
          _buildRAGRow('KB Coverage', '$RAG_KB_COVERAGE', '1.00',
              highlight: true),
          _buildRAGRow('Recall@5', '$RAG_RECALL_AT_5', '>=0.85',
              highlight: false),
          _buildRAGRow('MRR', '$RAG_MRR', '>=0.70', highlight: false),
          _buildRAGRow('NDCG@5', '$RAG_NDCG_AT_5', '>=0.70', highlight: false),
          _buildRAGRow('Embedding Drift', '$RAG_EMBEDDING_DRIFT', '< 0.2',
              highlight: false),
          ...
          Container(
            ...
            child: const Text(
              '* F1 = 1.0 is on synthetic template text (not real data). Each tea disease class uses a fixed keyword pattern, making classification trivially easy. The low Recall@5 = 0.129 is the honest real-world retrieval signal - the retriever learns pattern matching, not deep semantic alignment.',
          ...
          Container(
            ...
            child: const Text(
              'Privacy: estate-local FAISS stores never leave the device. Joint training uses InfoNCE + BCE losses. EMA aggregation (mu=0.9) prevents embedding drift.',
              style:
  ...
  Widget _buildRAGRow(String metric, String value, String target,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildDetailRow(
              'Data Split', 'Non-IID Dirichlet (alpha=$NON_IID_ALPHA)'),
          _buildDetailRow('Avg Fed/Cent Retention', '$FED_CENTRALIZED_RATIO%'),
          _buildDetailRow('Best Fed F1 (VLM-CLIP)', '$BEST_MACRO_F1_FEDERATED'),
          _buildDetailRow(
              'Best Cent F1 (VLM-CLIP)', '$BEST_MACRO_F1_CENTRALIZED'),
          _buildDetailRow(
              'VLM gain over LLM', '+$MULTIMODAL_IMPROVEMENT% Fed F1'),
