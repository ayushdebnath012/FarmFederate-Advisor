...
for crop stress detection and agricultural AI applications.
...
- Dataset comparison: AG News, PubMed Agriculture, SQUAD, PlantVillage, Beans, synthetic data
...
License: MIT
Version: 5.1 (Dataset Comparison + BTP Report Datasets Edition)
"""

from __future__ import annotations

# ============================================================================
# AUTO-INSTALL DEPENDENCIES (for Colab single-cell execution)
# ============================================================================
import sys
...
class Config:
    ...
    use_mixed_precision: bool = True  # FIXED v3: Enable mixed precision by default
...
def train_model(model, train_loader, val_loader, config: Config, device, model_type='text',
    ...
    return best_f1, history, metrics, best_model_state
...
def run_training(config: Config, allow_short: bool = False, skip_download: bool = False):
    ...
    # ==================== Dataset Comparison with Best Model ====================
    # NOTE: This must run BEFORE plot generation so plots include dataset comparison data
    print("\n" + "=" * 90)
    print("[6/8] RUNNING DATASET COMPARISON WITH BENCHMARK MODEL")
    print("=" * 90)

    try:
        # Find best VLM model (highest F1)
        best_vlm_name = max(results['vlm_models'].items(), key=lambda x: x[1]['f1'])[0]
        best_vlm_f1 = results['vlm_models'][best_vlm_name]['f1']
        print(f"Using best model: VLM-{best_vlm_name} (F1={best_vlm_f1:.4f})")

        dataset_comparison_results = compare_datasets_with_benchmark(config)
        results['dataset_comparison'] = dataset_comparison_results

        # Copy dataset results to top level for visualization compatibility
        if 'text_datasets' in dataset_comparison_results:
            results['text_datasets'] = dataset_comparison_results['text_datasets']
        if 'image_datasets' in dataset_comparison_results:
            results['image_datasets'] = dataset_comparison_results['image_datasets']
        if 'intra_comparison' in dataset_comparison_results:
            results['intra_comparison'] = dataset_comparison_results['intra_comparison']
        if 'inter_comparison' in dataset_comparison_results:
            results['inter_comparison'] = dataset_comparison_results['inter_comparison']

        # Save dataset comparison results
        dataset_results_file = config.output_dir / 'dataset_comparison_results.json'
        with open(dataset_results_file, 'w') as f:
            json.dump(dataset_comparison_results, f, indent=2, default=str)
        print(f"\n✅ Dataset comparison saved to: {dataset_results_file}")

    except Exception as e:
        print(f"\n⚠️  Dataset comparison failed: {e}")
        import traceback
        traceback.print_exc()
        ...
        try:
            ...
            for epoch in range(min(config.epochs, 5)):  # Quick comparison (5 epochs max)
                ...
                for i in range(0, len(train_texts), config.batch_size):
                    ...
                    # Tokenize (simplified)
                    input_ids = torch.zeros(len(batch_texts), 128, dtype=torch.long).to(device)
                    attention_mask = torch.ones(len(batch_texts), 128, dtype=torch.long).to(device)
...
def main():
    ...
    parser.add_argument('--compare-datasets', action='store_true',
                        help='Run dataset comparison using benchmark model (RoBERTa + Swin + BLIP-2)')
    ...
    if args.setup:
        ...
        setup_environment()
        print("[Info] Running dataset comparison with benchmark model...")
        results = compare_datasets_with_benchmark(config)
        print("[Info] Dataset comparison complete. Results saved.")
        return results
    elif args.train:
        setup_environment()
        run_training(config)
...
if __name__ == '__main__':
    ...
    # Check if we're in Colab/Jupyter - prioritize single-cell execution
    if IN_COLAB or 'ipykernel' in sys.modules:
        # Colab/Jupyter single-cell mode - use EXECUTION_MODE
        if EXECUTION_MODE == 'manual':
            # Show guidance, don't auto-run
            print("\n" + "="*70)
            print("🌾 FarmFederate v5.1 - MANUAL MODE")
            print("="*70)
            print("\nTo run training, execute one of these:\n")
            print("  run_quick_test()  # 2-3 minutes")
            print("  run_colab(epochs=12, max_samples=600)  # 30-60 minutes")
            print("\nOr change EXECUTION_MODE to 'quick', 'standard', or 'full' above.\n")
        else:
            # Auto-run based on mode
            print("\n" + "="*70)
            print("🚀 FarmFederate v5.1 - AUTO-RUNNING IN COLAB")
            print("="*70)
            print(f"\n📊 Mode: {EXECUTION_MODE.upper()}")
            print("📦 Training 24 models (5 LLM + 5 ViT + 8 VLM + 6 Federated)")

            if EXECUTION_MODE == 'quick':
                print("⏱️  Estimated time: 2-3 minutes (smoke test)\n")
                run_quick_test()
            else:
                config = TRAINING_CONFIG.get(EXECUTION_MODE, TRAINING_CONFIG['standard'])
                print(f"⏱️  Estimated time: {'30-60' if EXECUTION_MODE == 'standard' else '60-90'} minutes on T4 GPU\n")
                run_colab(
        ...
        print("Or set EXECUTION_MODE='standard' and run in Colab/Jupyter")
