...
class _AnalyticsScreenState extends State<AnalyticsScreen> {
  ...
  Widget _buildSummaryCards() {
    ...
    final avgMoisture =
        _historicalData.map((e) => e.soilMoisture).reduce((a, b) => a + b) /
            _historicalData.length;
    final avgTemp =
        _historicalData.map((e) => e.airTemperature).reduce((a, b) => a + b) /
            _historicalData.length;
    final avgHumidity =
        _historicalData.map((e) => e.humidity).reduce((a, b) => a + b) /
            _historicalData.length;
        ...
        Row(
          children: [
            ...
            Expanded(
              child: _buildStatCard(
                'Avg Temperature',
                '${avgTemp.toStringAsFixed(1)} deg C',
                Icons.thermostat,
                Colors.orange,
              ),
            ),
  ...
  Widget _buildCropHealthStatus() {
    ...
    final tempOk = latest.airTemperature >= tempRange[0] &&
        latest.airTemperature <= tempRange[1];
    final humOk =
        latest.humidity >= humRange[0] && latest.humidity <= humRange[1];
    final soilOk = latest.soilMoisture >= soilRange[0] &&
        latest.soilMoisture <= soilRange[1];
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildRangeRow(
            'Temperature',
            '${latest.airTemperature.toStringAsFixed(1)} deg C',
            '${tempRange[0].toInt()}-${tempRange[1].toInt()} deg C',
            tempOk,
          ),
  ...
  Widget _buildTrendAnalysis() {
    ...
    final avgMoist1 =
        firstHalf.map((e) => e.soilMoisture).reduce((a, b) => a + b) /
            firstHalf.length;
    final avgMoist2 =
        secondHalf.map((e) => e.soilMoisture).reduce((a, b) => a + b) /
            secondHalf.length;
    final moistTrend = avgMoist2 - avgMoist1;

    final avgTemp1 =
        firstHalf.map((e) => e.airTemperature).reduce((a, b) => a + b) /
            firstHalf.length;
    final avgTemp2 =
        secondHalf.map((e) => e.airTemperature).reduce((a, b) => a + b) /
            secondHalf.length;
    final tempTrend = avgTemp2 - avgTemp1;

    final avgHum1 = firstHalf.map((e) => e.humidity).reduce((a, b) => a + b) /
        firstHalf.length;
    final avgHum2 = secondHalf.map((e) => e.humidity).reduce((a, b) => a + b) /
        secondHalf.length;
      ...
      child: Column(
        ...
        children: [
          ...
          _buildTrendItem(
            ...
            moistTrend.abs() < 1
                ? Colors.blue
                : (moistTrend >= 0 ? Colors.green : Colors.orange),
          ),
          const SizedBox(height: 12),
          _buildTrendItem(
            'Temperature',
            '${tempTrend >= 0 ? "+" : ""}${tempTrend.toStringAsFixed(1)} deg C change',
            tempTrend.abs() < 1
                ? Icons.check_circle
                : (tempTrend >= 0 ? Icons.arrow_upward : Icons.arrow_downward),
            tempTrend.abs() < 1
                ? Colors.blue
                : (tempTrend > 3 ? Colors.red : Colors.orange),
          ),
          const SizedBox(height: 12),
          _buildTrendItem(
            'Humidity',
  ...
  Widget _buildPredictiveInsights() {
    ...
    if (ranges != null) {
      ...
      if (latest.soilMoisture < soilRange[0] + 5) {
        insights.add({
          ...
          'desc':
              'Soil moisture approaching lower limit for $_selectedCrop. Consider watering within 24 hours.',
        });
      } else if (latest.soilMoisture > soilRange[1] - 5) {
        insights.add({
          'title': 'Reduce Watering',
          'desc':
              'Soil is well hydrated. Skip next irrigation cycle to prevent waterlogging.',
        });
      }
      ...
      if (latest.airTemperature > tempRange[1] - 2) {
        insights.add({
          ...
          'desc':
              'Temperature nearing upper limit for $_selectedCrop. Consider shade covers or misting.',
        });
      }
    }
    ...
    if (latest.humidity > 85) {
      insights.add({
        'title': 'Disease Watch',
        'desc':
            'High humidity increases fungal disease risk. Ensure good airflow between plants.',
      });
    }
    ...
    if (insights.isEmpty) {
      insights.add({
        ...
        'desc':
            'Current readings are within optimal range for $_selectedCrop. Keep monitoring!',
      });
    }
