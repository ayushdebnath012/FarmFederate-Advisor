...
class Message {
  ...
  final Map<String, dynamic>? advisoryAnalysis;
...
class AdvisoryChatScreen extends StatefulWidget {
  ...
  const AdvisoryChatScreen({Key? key, required this.apiBase}) : super(key: key);
...
class _AdvisoryChatScreenState extends State<AdvisoryChatScreen>
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      ...
      setState(() {
        _messages.add(
          Message(
            text: _formatAdvisoryResponse(response),
            isUser: false,
            timestamp: DateTime.now(),
            advisoryAnalysis: response,
          ),
        );
        _isTyping = false;
      });
  ...
  String _formatAdvisoryResponse(Map<String, dynamic> response) {
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (message.advisoryAnalysis != null) ...[
                    const SizedBox(height: 8),
                    _buildAnalysisChips(message.advisoryAnalysis!),
                  ],
                  const SizedBox(height: 8),
                  Text(
