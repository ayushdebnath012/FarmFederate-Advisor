...
fig, ax = plt.subplots(figsize=(20.0, 14.0)) # Massively stretched HORIZONTALLY
im = ax.imshow(data_h, cmap='RdYlGn', vmin=0.38, vmax=1.0, aspect='auto')

cbar = plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
cbar.set_label('Score', fontsize=LABEL+4)
cbar.ax.tick_params(labelsize=TICK+2)

ax.set_xticks(range(len(metrics)))
ax.set_xticklabels(metrics, fontsize=TICK+4, fontweight='bold')
...
for i in range(len(row_labels)):
    for j in range(len(metrics)):
        ...
        ax.text(j, i, f'{val:.3f}', ha='center', va='center', fontsize=ANNOT, fontweight='bold', color=txt_col)

ax.axhline(4.5, color='white', linewidth=5.0)
ax.axhline(9.5, color='white', linewidth=5.0)

for grp_label, y_pos, col in [('LLM', 2.0, '#1565C0'), ('ViT', 7.0, '#E65100'), ('VLM', 13.5, '#B71C1C')]:
    ax.text(2.6, y_pos, grp_label, va='center', ha='left', fontsize=TICK+8, color=col, fontweight='bold', rotation=90)

ax.set_title('Performance Heatmap — All 18 Architectures\n(Tea Leaf Disease, Macro F1 / Precision / Recall)', fontsize=TITLE+6, fontweight='bold', pad=30)

for p in paths:
    try:
        plt.savefig(p / 'plot13_heatmap.png', dpi=300, bbox_inches='tight')
    except:
        pass
plt.close()
print("Done!")
